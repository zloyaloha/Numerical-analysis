...
FiniteDifferenceSolver::FiniteDifferenceSolver(const double& a_, const double& b_, const double& alpha_, const double& beta_, const double& eps_) : a(a_), b(b_), alpha(alpha_), beta(beta_), eps(eps_)
{
...
std::vector<Point> FiniteDifferenceSolver::solve(std::function<double(const double&, const double&, const double&)> f, const double& h)
{
    int N = (b - a) / h;
    Tridiagonal tr;
